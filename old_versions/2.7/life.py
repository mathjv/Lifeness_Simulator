...
class Activation: # Activador
    ...
    dst= os.path.join(path, "activation.json")
    src_readme= os.path.join(BASE_DIR, "docs", "activation.json")
    shutil.copyfile(src_readme, dst)

    def save_activation(user: str, key: str):
        json.loads(ACTIVATION_FILE.read_text(encoding="utf-8"))
        payload = {"user": user, "key": key}
        ACTIVATION_FILE.write_text(json.dumps(payload, ensure_ascii=False), encoding="utf-8")
...
class ReportGenerator:
    ...
    def generate(self) -> str:
        doc = Document(os.path.join(BASE_DIR, "docs", "report_template.docx"))
...
class MainWindow(QMainWindow): # Constructor o init
    def __init__(self, parser, meta: MetaProyecto):
        ...
        self.menu_buttons = [self.btn_sim, self.btn_book]
        self.act_buttons=[self.btn_change, self.btn_extras, self.btn_help]
        ...
        self.toggle_button.setGeometry(920, 0, 120, 30)  # posición aprox.
    ...
    def show_sim_categories(self):
        ...
        self.txt_oms.setHtml("<h2><center>Seleccione Sistema Inicial</center></h2>")
    ...
    def show_age_selection(self):
        self.clear_right_panel()
        self.clear_right_panel()
        self.txt_oms.setHtml("<h2><center>Seleccione la edad</center></h2>"
 ...
 #-------------------------FIN DE FUNCIONES DE SELECCION JERARQUICA
    ...
    def generate_report(self):
        ...
        path = rg.generate()
        answer = QMessageBox.question(self, "Life", f"Reporte listo. ¿Desea guardarlo en {path}?.", QMessageBox.Yes | QMessageBox.No)
        if answer == QMessageBox.Yes:
            QMessageBox.information(self, "Success", f"Excelente! Reporte exitosamente guardado a {path}.")
            dialog.accept()
...
class WelcomeScreen(QWidget):
    def __init__(self, main_window_class, user_name, first_time):
        ...
        self.setWindowTitle("Lifeness Welcome")
    ...
    def continuar(self):
        dialog = QDialog(self)
        QMessageBox.information(self, "Lifeness Simulator", "Welcome to Life Simulator. Happy Experience")
        dialog.exec()
        self.close()
...
class AppController: # Controla el flujo de la aplicación
    ...
    def show_welcome(self):
        first_time = not os.path.exists("first_launch.flag")
        ...
        if os.path.exists("activation.json"):  # Leer usuario registrado
            with open(BASE_LOG, "Lifeness Simulator", "activation.json", "r") as f:
                data = json.load(f)
                user = data.get("user", "")
        else:
            user=""
        self.welcome = WelcomeScreen(MainWindow, user, first_time)
        self.welcome.show()
