...
from PySide6.QtWidgets import (QApplication, QGraphicsOpacityEffect, QMainWindow, QLineEdit, QWidget, QLabel, QPushButton, QGraphicsDropShadowEffect, QListWidget, QTextEdit, QHBoxLayout, QVBoxLayout, QSplitter, QSlider, QMessageBox, QDialog, QFormLayout, QComboBox)
...
class GLHumanWidget(QOpenGLWidget):
    ...
    def load_model(self, model_path):     
        if not os.path.isfile(model_path):
            ...
            return
        try:
            self.current_model = OBJ(model_path) if os.path.isfile(self.model_path) else None
            self.update()
...
class DiseasePatogen(QMainWindow):
    def __init__(self, enfermedad_actual, descripciones):
        ...
        label_title_2 = QLabel("P")
        label_title_2.setFont(QFont("Segoe UI", 10, QFont.Bold))
...
class AuthorsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.close_btn = AnimatedButton("Enviar gradecimientos y Salir")
...
class MainWindow(QMainWindow): # Constructor o init
    ...
    def show_disease_selection(self):
        ...
        self.enfermedades_descripcion = {
            "COVID-19": "El COVID-19, abreviatura de Coronavirus Disease 2019, es una enfermedad respiratoria causada por el virus SARS-CoV-2, perteneciente a la familia de los coronavirus. Fue identificada por primera vez en Wuhan, China, en diciembre de 2019. Su origen se asocia al salto zoonótico de un virus de murciélago hacia humanos, probablemente a través de un hospedador intermedio.\nLa enfermedad se propagó rápidamente, convirtiéndose en una pandemia global declarada por la OMS el 11 de marzo de 2020.\nSu mecanismo principal afecta al sistema respiratorio, causando fiebre, tos seca, dificultad para respirar, pérdida del olfato y gusto, y en casos graves, neumonía, síndrome de dificultad respiratoria aguda y fallo multiorgánico.",
            "Hepatitis": "La hepatitis es la inflamación del hígado, órgano esencial encargado de filtrar toxinas y metabolizar nutrientes. Su nombre proviene del griego hepar (hígado) y itis (inflamación). Existen varios tipos: A, B, C, D y E, cada uno con un agente viral distinto y diferentes formas de transmisión.\nHepatitis A y E: transmitidas por alimentos o agua contaminados.\nHepatitis B, C y D: por contacto con sangre o fluidos corporales infectados.\nLa enfermedad puede ser aguda o crónica. En sus formas graves puede provocar cirrosis o cáncer hepático.\nLos síntomas incluyen ictericia (color amarillento de la piel), fatiga, náuseas y dolor abdominal.",
            "Anemia": "La anemia es una deficiencia en la cantidad o calidad de glóbulos rojos o hemoglobina, lo que reduce la capacidad de transporte de oxígeno en la sangre. Su origen puede ser nutricional (falta de hierro, vitamina B12 o ácido fólico), genético (como la anemia falciforme) o secundario a enfermedades crónicas.\n\nSe conoce desde la antigüedad, descrita ya por médicos griegos y egipcios. Los pacientes suelen mostrar palidez, cansancio, mareos, taquicardia y dificultad para concentrarse.",
            ...
            "Esclerosis múltiple": "La esclerosis múltiple (EM) es una enfermedad autoinmune y degenerativa del sistema nervioso central. El propio sistema inmunitario ataca la mielina, sustancia que recubre las fibras nerviosas, interrumpiendo la comunicación entre cerebro y cuerpo.\n\nDescubierta en el siglo XIX por Jean-Martin Charcot, su causa sigue sin conocerse del todo, aunque se asocia a predisposición genética, infecciones virales y factores ambientales.",
            ...
            "Dermatitis": "La dermatitis es una inflamación de la piel causada por factores alérgicos, irritantes o inmunológicos. Su término deriva del griego derma (piel) y itis (inflamación). Puede ser atópica, seborreica, de contacto o por irritación.\n\nSus síntomas incluyen enrojecimiento, picazón, descamación y, a veces, ampollas o grietas. Tiene un fuerte componente genético y ambiental: productos químicos, detergentes, polvo o estrés pueden desencadenarla.",
    ...
    def on_model_slider_changed(self, value):
        if value < 33:
            ...
            models = "male_muscle.obj"
        else:
            models = "male_skeleton.obj"

        model_path = os.path.join(ASSETS_DIR, "anatomy", models)
